...
#include <queue>
using namespace std;
typedef pair<int, BWAPI::UnitType> p;
...
void StarterBot::sendIdleWorkersToMinerals()
{
    ...
    for (auto& unit : myUnits)
    {
        ...
        if (unit->getType().isWorker() && unit->isIdle())// 如果让农民一直点矿，就会什么都不干
        {
...
void StarterBot::buildAdditionalSupply()
{
    ...
    //包括还没有建好的supply的人口(supply)，以及如果有农民正在去建，但还没开始建  
    ...
    const bool startedBuilding = Tools::BuildBuilding(supplyProviderType);// 返回建筑是否开始建造
    if (startedBuilding)
    {
...
//建筑顺序
//策略初始化， 10人口兵营， 14人口 Cybernetics Core， 29人口 Nexus
void StarterBot::initialStrategy() {
    ...
    building_order.push(make_pair(58, BWAPI::UnitTypes::Protoss_Nexus));
}
//建造
void StarterBot::build() {
    ...
    if (!building_order.empty()) {
        ...
        if (supply == b1.first) {//人口够了
            const bool startedBuilding = Tools::BuildBuilding(b1.second);// 返回建筑是否开始建造
            if (startedBuilding)
            {
                BWAPI::Broodwar->printf("Started Building %s", BWAPI::UnitTypes::Protoss_Gateway.getName().c_str());
            }
            building_order.pop(); //这个建筑从队列中扔掉
        }
    }
}
