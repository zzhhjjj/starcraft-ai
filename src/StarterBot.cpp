...
void StarterBot::onFrame()
{
    ...
    AttackGroup attackGp = m_meleeManager.getCombatGroup(m_data.CombatRallyPoint, 128 * 32, 3);
    //BWAPI::BroodwarPtr->printf("Attack group formed: %d", attackGp.size());
    if (attackGp.size() == 3) { m_meleeManager.attackBase(attackGp, m_data.enemy_base); }
    //end update
}
...
void StarterBot::onUnitComplete(BWAPI::Unit unit)
{
    ...
    if (unit->getType() == BWAPI::UnitTypes::Protoss_Pylon && m_data.front_pylon) {
        if (unit->getDistance(BWAPI::Position(m_data.front_pylon_pos)) < 32 * 128) {
            m_data.second_pylon = false;
...
void StarterBot::onUnitShow(BWAPI::Unit unit)
{
    ...
    if (unit->getPlayer()->isEnemy(BWAPI::Broodwar->self()) == true) {
        ...
        if (unit->getType().isBuilding() && m_data.detecte_enemy == false) {
            ...
            m_data.front_pylon_pos = BWAPI::TilePosition(BWAPI::Position(1.8 * scouter_pos.x - 0.8 * enemy_building_pos.x, 1.8 * scouter_pos.y - 0.8 * enemy_building_pos.y)).makeValid();
           /* while (!m_mapTools.isBuildable(m_data.front_pylon_pos)) 
            {
                m_data.front_pylon_pos = BWAPI::TilePosition(BWAPI::Position(1.1 * m_data.front_pylon_pos.x - 0.1 * enemy_building_pos.x, 1.1 * m_data.front_pylon_pos.y - 0.1 * enemy_building_pos.y)).makeValid();
            }*/
            //m_scout->move(BWAPI::Position(1.8 * scouter_pos.x - 0.8 * enemy_building_pos.x, 1.8 * scouter_pos.y - 0.8 * enemy_building_pos.y));
            m_scout->move(BWAPI::Position(m_data.home));
...
void StarterBot::front_strategy() {
    if (m_data.detecte_enemy) {
        ...
        if (m_data.front_canon < 2 && m_data.front_pylon && m_data.current_mineral() > BWAPI::UnitTypes::Protoss_Photon_Cannon.mineralPrice()) {
        ...
        if (m_data.front_gateway > 0 && m_data.second_pylon == false && m_data.current_mineral() > BWAPI::UnitTypes::Protoss_Pylon.mineralPrice()) {
            BWAPI::Broodwar->printf("want to build front pylon  %d", m_data.current_mineral());
            BWAPI::Position wanted_pos = BWAPI::Position(0.8 * BWAPI::Position(m_data.front_pylon_pos).x + 0.2 * m_data.enemy_base.x, 0.8 * BWAPI::Position(m_data.front_pylon_pos).y + 0.2 * m_data.enemy_base.y).makeValid();
        ...
        if (m_data.second_pylon && m_data.front_canon<8 && m_data.current_mineral() > BWAPI::UnitTypes::Protoss_Photon_Cannon.mineralPrice()) {
            ...
            BWAPI::Position wanted_pos = BWAPI::Position(0.9 * BWAPI::Position(m_data.front_pylon_pos).x + 0.1 * m_data.enemy_base.x, 0.9 * BWAPI::Position(m_data.front_pylon_pos).y + 0.1 * m_data.enemy_base.y).makeValid();
            BWAPI::TilePosition cannonpos = BWAPI::Broodwar->getBuildLocation(BWAPI::UnitTypes::Protoss_Photon_Cannon, BWAPI::TilePosition(wanted_pos), 64, false);
            bool  succeed = m_scout->build(BWAPI::UnitTypes::Protoss_Photon_Cannon, cannonpos);
            if (succeed) {
                m_data.current_minus_mineral += BWAPI::UnitTypes::Protoss_Photon_Cannon.mineralPrice();
